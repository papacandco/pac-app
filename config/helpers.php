...
if (! function_exists('get_technologies')) {
    ...
    function get_technologies()
    {
        ...
        if (is_null($technologies)) {
            $technologies = \App\Models\Technologie::where('parent_id', 0)->get();
        }
    ...
    function get_latest_comment($context = null, $take = 5)
    {
        ...
        if (is_null($context)) {
            return \App\Models\Comment::take($take)
                ->withFlash(['commentable', 'user'])->get();
        }

        return \App\Models\Comment::take($take)
            ->whereIn('commentable_type', [Question::class, Tutorial::class, Podcast::class])
            ->withFlash(['commentable', 'user'])->get();
    }
}
...
if (! function_exists('str_words')) {
    ...
    {
        return Str::getWords($value, $words) . $end;
    }
}
...
if (! function_exists('gravatar')) {
    ...
    {
        if (is_null($email)) {
            $email = 'contact@codelearningclub.com';
        }
