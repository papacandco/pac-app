...
@forelse(get_latest_comment() as $comment)
  @continue(is_null($comment->commentable))
  @continue(is_null($comment->user))
  <p class="shadow-sm" style="padding: 10px; word-wrap: break-word; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
    ...
    <span class="text-muted">
      ...
      <a href="{{ route('challenge.single', [
          'slug' => $comment->commentable->slug,
          'challenge' => $comment->commentable->id
        ]) }}#c-{{ $comment->id }}"
        style="text-decoration: underline;">
        {{ $comment->commentable->title }}
      </a>
      @elseif($comment->isForQuestion())
      <a href="{{ route('forum.question', [
          'slug' => $comment->commentable->slug,
          'id' => $comment->commentable->id
        ]) }}#c-{{ $comment->id }}"
        style="text-decoration: underline;">
      ...
      <a href="{{ route('podcast.single', [
          'slug' => $comment->commentable->slug,
          'podcast' => $comment->commentable->id
        ]) }}#c-{{ $comment->id }}"
        style="text-decoration: underline;">
        {{ $comment->commentable->title }}
      </a>
