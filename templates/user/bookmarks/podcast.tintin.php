...
<section class="container" style="margin-bottom: 150px">
  <h3>{{ __('user.bookmark.podcast.title') }}</h3>
  <div class="row">
    <aside class="col-sm-2 hidden-xs" style="margin-top: 10px;">
      %include('user.partials.sidebar', ["active" => "podcast"])
    </aside>
    <div class="col-sm-10">
      <div class="row">
        %forelse($bookmarks as $podcast)
          %include('podcast.partials.podcast', ['podcast' => $podcast])
        %empty
          %include('partials.no-data', ['message' => __('user.bookmark.podcast.no_data')])
        %endforelse
      </div>
    </div>
  </div>
