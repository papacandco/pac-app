...
%block('seo')
  %include('partials.seo', ['title' => $podcast->title, 'description' => $podcast->description, 'image' => $podcast->cover, 'author' => $podcast->author->name])
%endblock
...
%block('content')
  <div class="jumbotron jumbotron-gray text-left" style="padding-bottom: 10px">
    ...
    <div class="container video-mobile">
      %if($podcast->isVideo())
        %include('partials.video', ['url' => $podcast->getSource(), 'mode' => 'video'])
      %else
        %include('partials.audio', ['source' => $podcast->getSource()])
      %endif
    </div>
  </div>
  ...
  <section class="container">
    <div class="row">
      <aside class="col-sm-1 hidden-xs" style="margin-top: 10px;">
        %include('partials.social-sidebar', ["url" => route('podcast.single', ['id' => $podcast->id, 'slug' => str_slug($podcast->title)])])
      </aside>
      ...
      <aside class="col-sm-3" style="margin-top: 10px;">
        ...
        <div class="row">
          <div class="col-sm-12">
            ...
            <p>
              <img src="{{ gravatar($podcast->author->avatar) }}" class="img-circle img-responsive" alt="Favoris" style="width: 25px; height: 25px; display: inline-block;">
  ...
  %auth
    %include('tutorial.modal.bookmark', ['url' => route('bookmark', ['id' => $podcast->id]), 'type' => 'podcast', 'message' => 'Voulez-vous ajouter ce podcast dans votre liste de favoris'])
  %endauth
%endblock
