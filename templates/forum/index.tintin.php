...
%block('seo')
  %include('partials.seo', ['title' => "Forum", 'description' => "Ici, Vous pouvez poser votre question relative à chaque thématique"])
%endblock
...
<section class="container">
  <div class="row">
    ...
    <section class="col-sm-8 col-xs-12" style="margin-top: 10px;">
      ...
      <div class="row no-gutters nogutters">
        %loop($questions->items() as $question)
          %include('forum.partials.question', ['question' => $question, 'type' => 'forum'])
        %endloop
        %if($questions->items()->count() == 0)
          %include('partials.no-data', ['message' => __('forum.question-no-data')])
        %endif
      </div>
      {## <div class="row no-gutters nogutters">
