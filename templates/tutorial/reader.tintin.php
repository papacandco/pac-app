%extends('layouts.app', ['active' => $form_curriculum ? 'curriculum' : 'tutorial'])

%block('title', $tutorial->title . ' - tutoriel')

%block('seo')
  %include('partials.seo', ['title' => $tutorial->title,'description' => $tutorial->description, 'image' => $tutorial->cover,'author' => $tutorial->author->name])
%endblock
...
%block('content')
      ...
      <div class="col-sx-12 col-sm-9" style="margin-top: 10px;">
        ...
        <div class="row">
          <div class="col-sm-12">
            <div class="pull-right">
              ...
              %else
                <a class="hidden-xs" style="position: relative; top: 20px" href="{{ route('login', ['redirect' => request()->url()]) }}" title="Ajoutez dans vos favoris">
                  <img class="no-img-bg-color" src="/img/misc/tag.svg" alt="Favoris" style="width: 18px; position: relative; top: -1px;">
                </a>
              %endauth
            </div>
        ...
        <div style="margin-top: 50px; margin-bottom: 20px">
          ...
          <div style="margin-top: 10px;">
            %include('tutorial.comment.form', ['route' => route('comment.make', [ 'id' => $tutorial->id, 'type' => 'tutorial'])])
          </div>
          <div style="margin-top: 30px">
            %include('tutorial.partials.comment', ['comments' => $tutorial->comments, 'type' => 'tutorial', 'target' => $tutorial])
          </div>
        </div>
      </div>
        ...
        %if($form_curriculum)
          <div class="row">
            %auth
              %if($followed)
              <div class="col-sm-12">
                %if(!$tutorial->hasProgress(auth()->user(), $curriculum->id))
            ...
            %endauth
            <div class="col-sm-12">
              %include('partials.forum', ['questions' => $curriculum->lastQuestion(3), 'title' => $curriculum->title, 'simple_form' => true, 'forum_link' => route('forum.single', ['slug' => $curriculum->slug, 'id' => $curriculum->id, 'type' => 'curriculum'])])
            </div>
          </div>
        %endif
        <div class="row">
  ...
  %auth
    ...
    %include('tutorial.modal.bookmark', ['url' => route('bookmark', ['id' => $tutorial->id]), 'type' => 'tutorial', 'message' => 'Voulez-vous ajouter ce tutoriel dans votre liste de favoris'])
  %endif
